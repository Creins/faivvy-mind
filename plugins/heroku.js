...
ibraal  {
  ...
  async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      if (!superUser) {
        return repondre(`CREINS-MD-XFORCE says only owner or Fredie can use this command 🚫`);
      }

      if (!arg[0] || !arg.join(' ').includes('=')) {
        return repondre(`CREINS-MD-XFORCE\n\n╭┈┈┈┈┈┈┈┈┈┈┈┈\n│❒ Use this Format it right, like: .setvar OWNER_NUMBER=254787243487\n╰┈┈┈┈┈┈┈┈┈┈┈┈`);
      }
      ...
      if (!key || !value) {
        return repondre(`CREINS-MD-XFORCE says STOP WASTING MY TIME! Provide a valid KEY=VALUE pair!🙂‍↔️`);
      }

      if (!s.HEROKU_API_KEY || !s.HEROKU_APP_NAME) {
        return repondre(`CREINS-MD-XFORCE says CONFIG ERROR! HEROKU_API_KEY or HEROKU_APP_NAME missing in set.js! Fix it now!`);
      }
      ...
      await repondre(`CREINS-MD-XFORCE\n\n╭┈┈┈┈┈┈┈┈┈┈┈┈\n│❒ BOOM! Heroku var ${key} set to ${value}! bot is rebooting...🛒\n╰┈┈┈┈┈┈┈┈┈┈┈┈`);

    } catch (error) {
      console.error('setvar error:', error);
      await repondre(`CREINS-MD-XFORCE FAIL! Something broke: ${error.message} 😴 Fix it or suffer!`);
    }
  }
);
...
ibraal(
  ...
  async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      if (!s.HEROKU_API_KEY || !s.HEROKU_APP_NAME) {
        return repondre(`CREINS-MD-XFORCE CONFIG DISASTER! HEROKU_API_KEY or HEROKU_APP_NAME missing in set.js! Sort it out! 🙂‍↕️`);
      }
